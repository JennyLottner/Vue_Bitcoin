...
async function query(filterBy = null) {
    ...
    if (filterBy) {
        ...
        contacts = contacts.filter(contact =>
            ['name', 'phone', 'email'].some(field => regex.test(contact[field]))
        )
    }

    contacts = contacts.sort((a, b) => a.name.toLocaleLowerCase().localeCompare(b.name.toLocaleLowerCase()))
    return contacts
}
...
async function save(contact) {
    if (contact._id) return await storageService.put(KEY, contact)
    else return await storageService.post(KEY, contact)
}
